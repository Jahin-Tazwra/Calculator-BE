...
async function analyzeImage(imgPath, dictOfVars) {
  try {
    ...
    6. Detecting any quadratic equations, first rearrange it as ax^2 + bx + c = 0 where a is the coefficient of x^2, b is the coefficient of x, and c is the constant. Then follow the next step and give both Real and Imaginary solutions.
    7. Detecting any quadratic equations like x^2 + x + 1 = 0 , solve for the given variable with quadratic formula and solve for Real and imaginary numbers. The answer should be a COMMA SEPERATED LIST OF DICTS, with dict 1 as {"expr": "x", "result": "(-1 + √3 * i) / 2", "assign": true} and dict 2 as {"expr": "x", "result": "(-1 - √3 * i) / 2", "assign": true}. This example assumes x was calculated as (-1 + √3 * i) / 2 and (-1 - √3 * i) / 2. Inclued as many dicts as there are variables.
